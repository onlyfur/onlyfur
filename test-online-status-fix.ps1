...
try {
    ...
    Write-Host "   ✓ Online status endpoint responded: Status $($response.StatusCode)" -ForegroundColor Green
} catch {
    $statusCode = $_.Exception.Response.StatusCode.value__
    if ($statusCode -eq 401) {
        Write-Host "   ✓ Online status endpoint correctly returned 401 Unauthorized" -ForegroundColor Green
    } else {
        Write-Host "   ⚠ Online status endpoint error: $($_.Exception.Message)" -ForegroundColor Yellow
    }
}
...
if (Test-Path $indexPath) {
    ...
    if ($jsFiles.Count -gt 0) {
        Write-Host "   ✓ JavaScript assets found: $($jsFiles.Count) files" -ForegroundColor Green
    } else {
...
Write-Host "`nIf you're still seeing ERR_INSUFFICIENT_RESOURCES errors:" -ForegroundColor Yellow
