import { neuralIndex } from '../data/neuralIndex';
...
// Utility to get all real help article slugs and real page routes (build-time, not runtime in browser)
const realHelpArticles = new Set([
  'FirstSubscription', 'LoginTroubleshooting', 'HowToCreateAccount', 'how-to-create-account', 'first-subscription', 'FindingCreators', 'finding-creators', 'EmailVerification', 'CustomCommissions', 'custom-commissions', 'CreatorEarnings', 'creator-earnings', 'MessageLimits', 'messaging-creators', 'message-limits', 'CreateAccount', 'ContentProtection', 'ContentPrivacyLevels', 'content-protection', 'content-privacy-levels', 'CommunityGuidelines', 'community-guidelines', 'BulkMessaging', 'bulk-messaging', 'BrowserCompatibility', 'browser-compatibility', 'AgeVerification', 'age-verification', 'AdvancedSearchFeatures', 'AccountSecurity', 'account-security', 'NeuralSearch', 'OAuthGuide', 'PasswordReset', 'PaymentMethods', 'payment-methods', 'PaymentSystem', 'payment-system', 'PricingStrategies', 'pricing-strategies', 'PrivacySettings', 'ProfileSetup', 'RefundPolicy', 'refund-policy', 'ReportUserContent', 'report-user-content', 'SchedulingFeatures', 'scheduling-features', 'SessionManagement', 'understanding-analytics', 'MobileApp', 'mobile-app', 'MessagingTips', 'MessagingCreators', 'UploadTroubleshooting', 'UploadOrganizeContent', 'SupportedFormats', 'SubscriptionTiersOverview', 'SubscriptionTiersGuide', 'SubscriptionTiers', 'SubscriptionManagement', 'subscription-management', 'setting-up-creator-profile', 'SettingUpCreatorProfile', 'TaxInformation', 'TwoFactorAuthentication', 'UnderstandingAnalytics', 'video-quality', 'VideoQuality', // ...add all others from file search
]);

const realPages = [
  '/explore', '/explorev3', '/earnings', '/dashboard', '/userprofile', '/termsofservice', '/subscriptionsettings', '/subscribe', '/register', '/profilev3', '/profile', '/privacypolicy', '/messagingv3', '/messagesold', '/messages', '/login', '/landingold', '/landing', '/howitworks', '/homev3', '/helpcenter', '/googlecallback', '/about', // ...add all others from file search
];
...
class NeuralSearchEngine {
  ...
  private initializeVectors(): void {
    this.vectors.clear();
    for (const entry of neuralIndex) {
      if (this.isRealPageOrArticle(entry, realPages, realHelpArticles)) {
        const content = [
          entry.description,
          entry.title,
          entry.id,
  ...
  private isRealPageOrArticle(entry: any, realPages: string[], realHelpArticles: Set<string>): boolean {
    // Accept if the url matches a real page route (case-insensitive)
    if (realPages.includes(entry.url.toLowerCase())) return true;
    // Only allow help articles that have a real file
    if (entry.url.startsWith('/help/articles/')) {
      const slug = entry.url.split('/').pop();
      return !!realHelpArticles && realHelpArticles.has(slug);
    }
    return false;
  }

  // Generates a random embedding for demo purposes
